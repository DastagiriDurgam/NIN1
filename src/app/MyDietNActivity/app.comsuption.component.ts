...
export class ConsuptionComponent {
  ...
  getData() {
    ...
    this.dbservice.createTable('recipies.sql', createQuery);
    ...
    this.dbservice.insertValuesToTable('recipies.sql', insertQuery, `recipes`);

    let getRecipiesQuery = "SELECT uid_recipes, Itemname, one_serving_wt_g FROM recipes";

    this.dbservice.getDataFromTable('recipes', 'recipies.sql', getRecipiesQuery);
