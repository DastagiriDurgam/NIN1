...
export class ConsuptionComponent {
    ...
    this.dbservice.getDatabaseState().subscribe(rdy => {
      if (rdy) {
        this.dbservice.getDataFromTable2(getRecipiesQuery).then(data => {
          ...
          this.totalRecipies = data.values;
  ...
  addRecipe(recipe, event, quantity, timetype, date) {
    if ((event.value.length != 0) && (date.length != 0)) {
      let filterVlues = this.selectedRecipies.filter((filterItem) => { let isSameItem = (filterItem.item[1] == this.currentitem[1]); let isSameTime = (timetype == filterItem.timetype); return isSameItem && isSameTime });
      this.isDisplayRecipeList = false;
